...
class User(AbstractUser):
    ...
    name = models.CharField(_("ФИО"), blank=True, max_length=255)
    picture = models.ImageField(
        _('Картинка профиля'), upload_to='profile_pics/', null=True, blank=True)
    location = models.CharField(
        _('Адрес'), max_length=50, null=True, blank=True)
    job_title = models.CharField(
        _('Название предприятия'), max_length=50, null=True, blank=True)
    personal_url = models.URLField(
        _('Сайт компании'), max_length=555, blank=True, null=True)
    ...
    bio = models.CharField(
        _('О себе'), max_length=280, blank=True, null=True)
